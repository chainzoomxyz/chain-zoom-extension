...
export const BuyForm = (props: TradingFormProps) => {
  ...
  const currentWalletBalance = _.get(balances, 'nativeBalance', 0);
  ...
  const { data: swapRouteData, isLoading: isLoadingSwapRoute } = useSwapRoute({
    ...
    tokenInAddress: NETWORK_CONFIG?.NATIVE_TOKEN_ADDRESS,
    tokenInDecimals: NETWORK_CONFIG?.DECIMALS.toString(),
    tokenOutAddress: _.get(tokenInfo, 'token.address', ''),
    tokenOutDecimals: tokenDecimals.toString(),
    amountIn,
  ...
  useEffect(() => {
    if (NETWORK_CONFIG.TYPE === NON_EVM_WALLET_NETWORK.SOLANA) {
      ...
      setValue(
        'amountOut',
        roundBalance(parseFloat(ethers.utils.formatUnits(outAmount, tokenDecimals).toString()), 5),
      );
    } else {
      setValue(
        'amountOut',
        roundBalance(
          parseFloat(
            ethers.utils
              .formatUnits(_.get(swapRouteData, 'routeSummary.amountOut', '0'), tokenDecimals)
              .toString(),
          ),
          5,
        ),
  ...
  const handleChangeAmountIn = (value: string) => {
    const parseUnits =
      NETWORK_CONFIG.TYPE === NON_EVM_WALLET_NETWORK.SOLANA
        ? value
        : ethers.utils.parseUnits(value || '0', NETWORK_CONFIG?.DECIMALS);
    // const parseUnits = ethers.utils.parseUnits(value || '0', 9);
  ...
  return (
    ...
    >
            ...
            <FormControl fullWidth>
              <Stack direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
                ...
                <Typography variant={'body2'} color={'common.white'}>
                  Balance: {currentWalletBalance} {NETWORK_CONFIG?.NATIVE_CURRENCY}
                </Typography>
              </Stack>
              <RadioGroup
              ...
              >
                ...
                >
                  <FormControlLabel
                    value="0.1"
                    disabled={currentWalletBalance <= 0.1}
                ...
                >
                  <FormControlLabel
                    value="0.25"
                    disabled={currentWalletBalance <= 0.25}
                    control={<Radio size={'small'} />}
                    label="0.25"
                  />
                </Box>
                ...
                >
                  <FormControlLabel
                    value="0.5"
                    disabled={currentWalletBalance <= 0.5}
                    control={<Radio size={'small'} />}
                    label="0.5"
                  />
                </Box>
              ...
              >
                ...
                <Box flex={1} height={1}>
                  <TextField
                    ...
                    type={'number'}
                    fullWidth
                    placeholder={'Custom Amount, i.e 100'}
                  />
                </Box>
              </Stack>
            </FormControl>
      ...
      <Button
        disabled={isLoading || isSubmitting || isLoadingSwapRoute || isLoadingBalance}
        variant={'soft'}
        color={'success'}
        size={'large'}
        type={'submit'}
      >
        {isLoading || isSubmitting || isLoadingSwapRoute || isLoadingBalance ? (
